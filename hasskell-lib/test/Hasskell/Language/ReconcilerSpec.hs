...
spec = do
  describe "Reconciler" $ do
    specify "can identify when a world needs no reconciliation" $
      property $ do
        observed <- forAll genObservedWorld
        let desired = case observed of
              MkObserved _ world -> MkDesired world
            MkReconciliationPlan steps = reconcile observed desired
        steps === []

    specify "does not create an empty plan for world that needs changes" $
      property $ do
        (offEntity, observed) <- forAll $ genWorldWithToggled Off
        let desired = observed `withEntityOn` offEntity
            MkReconciliationPlan steps = reconcile observed desired
        steps /== []
